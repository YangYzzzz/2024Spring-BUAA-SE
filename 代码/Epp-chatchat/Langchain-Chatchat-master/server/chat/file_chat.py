...
from server.chat.utils import print_2_log
import json
import os
...
def delete_temp_docs(
    knowledge_id: str = Form(..., description="临时知识库ID"),
    nothing_todo: str = Form("这是一个没用的属性", description="这是一个没用的属性")
):
    if knowledge_id not in memo_faiss_pool.keys():
        return BaseResponse(code=404, msg=f"未找到临时知识库 {knowledge_id}")
    else:
        memo_faiss_pool.pop(knowledge_id)
        return BaseResponse(code=200, msg=f"成功删除临时知识库 {knowledge_id}")
...
def upload_temp_docs(
    ...
    for success, file, msg, docs in _parse_files_in_thread(files=files,
        ...
        if success:
            documents += docs
            print_2_log(f'File: {file}, msg: {msg}, docs: {docs}')
        else:
            failed_files.append({file: msg})

    # 所有临时文件都在缓存内
    with memo_faiss_pool.load_vector_store(id).acquire() as vs:
        print_2_log(documents)
        vs.add_documents(documents)  # 先建立临时向量库，后往里写入内容
...
async def file_chat(query: str = Body(..., description="用户输入", examples=["你好"]),
    ...
    print_2_log(f'{query} {knowledge_id}')
    history = [History.from_data(h) for h in history]
    print_2_log(history)
    async def knowledge_base_chat_iterator() -> AsyncIterable[str]:
        nonlocal max_tokens, top_k
