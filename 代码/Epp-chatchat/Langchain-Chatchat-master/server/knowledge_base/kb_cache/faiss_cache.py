...
class _FaissPool(CachePool):
    def new_vector_store(
        ...
        print_2_log(f'doc: {doc}, embedding: {embeddings} 111')
        ...
        if cache is None:
            ...
            with item.acquire(msg="初始化"):
                ...
                if os.path.isfile(os.path.join(vs_path, "index.faiss")):
                    ...
                    vector_store.save_local(vs_path)  # 创建新的向量库并保存到本地磁盘
                else:
...
class MemoFaissPool(_FaissPool):
    def load_vector_store(
        ...
        if cache is None:
            ...
            # set了一个，若超过缓存大小，则会pop掉一个
            self.set(kb_name, item)
            with item.acquire(msg="初始化"):
                self.atomic.release()
                logger.info(f"loading vector store in '{kb_name}' to memory.")
                # create an empty vector store
                print_2_log(f'Embed model: {embed_model}, embed_device: {embed_device}')
                vector_store = self.new_vector_store(embed_model=embed_model, embed_device=embed_device) ##### 有问题
                item.obj = vector_store
                item.finish_loading()
        else:
            self.atomic.release()
